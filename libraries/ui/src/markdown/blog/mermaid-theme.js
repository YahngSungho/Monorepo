...
export function initMermaidTheme_action(mode) {
    ...
    const shouldWaitForCss = lastMode !== undefined && lastMode !== mode
	...
	globalInitPromise = (async () => {
		// DOM 파싱이 아직 진행 중일 때만 DOMContentLoaded를 기다림
        ...
        // 테마 토글 직후에만 CSS 변수 반영 시점을 보장하기 위해 다음 프레임(들) 대기
        if (shouldWaitForCss) {
            await new Promise((resolve) => {
		...
		// :root 요소에 접근하여 현재 테마에 맞는 CSS 변수 값들을 가져옴 (토글 반영 이후 시점)
