...
{#snippet template(arguments_)}
	...
	<ConfettiButtonDecorator {...arguments_}>
		{#snippet children()}
			<button
				...
				style:border="var(--border-size-1) solid var(--border)"
			>
				클릭해서 축하하기
			</button>
		{/snippet}
...
{#snippet longLabelTemplate(arguments_)}
	<ConfettiButtonDecorator {...arguments_}>
		{#snippet children()}
			<button
				style:padding="var(--space-s)"
...
{#snippet customColorsTemplate(arguments_)}
	<ConfettiButtonDecorator {...arguments_}>
		{#snippet children()}
			<button
				style:padding="var(--space-s)"
...
<!-- HAPPY PATH: 파라미터 튜닝 -->
<Story
	name="기본_활성화_조정(size_amount)"
	args={{ amount: 120, isConfettiActivated: true, size: 12 }}
	{template}
/>

<!-- BOUNDARY: 최소값/기본값 근처 -->
<Story name="경계값_최소_개수" args={{ amount: 1, isConfettiActivated: true }} {template} />

<!-- BOUNDARY: 큰 값들 -->
<Story
	name="경계값_큰_크기와_낙하거리"
	args={{ fallDistance: '600px', isConfettiActivated: true, size: 40 }}
	{template}
/>
...
<Story
	name="엣지_무한반복"
	args={{ destroyOnComplete: false, infinite: true, isConfettiActivated: true }}
	{template}
/>
...
<Story
	name="엣지_무중력_원뿔형"
	args={{ cone: true, isConfettiActivated: true, noGravity: true, xSpread: 0.8 }}
	{template}
/>
...
<Story
	name="잘못된_입력_colorRange_문자열"
	args={{ colorRange: 'not-array', isConfettiActivated: true }}
	{template}
/>

<!-- INVALID INPUT: 배열 길이/형식 문제 -->
<Story
	name="잘못된_입력_delay_스칼라"
	args={{ delay: 100, isConfettiActivated: true }}
	{template}
/>

<!-- ERROR HANDLING: reduce-motion 사용자 비활성화 옵션 확인 -->
<Story
	name="에러대응_reduceMotion_비활성화"
	args={{ disableForReducedMotion: true, isConfettiActivated: true }}
	{template}
/>
...
<Story
	name="스트레스_매우_많은_콘페티"
	args={{ amount: 1000, duration: 2500, isConfettiActivated: true, size: 8 }}
	{template}
/>
...
<Story
	name="스트레스_아주_긴_라벨"
	args={{ amount: 80, isConfettiActivated: true }}
	template={longLabelTemplate}
/>
...
<Story
	name="커스텀_색상_배열"
	args={{
		colorArray: ['#ff006e', '#8338ec', '#3a86ff', '#fb5607', '#ffbe0b'],
		isConfettiActivated: true,
	}}
	template={customColorsTemplate}
/>

<!-- POSITION/RANGE: x/y 범위 변경으로 분포 확인 -->
<Story
	name="분포_확대_좁힘"
	args={{ isConfettiActivated: true, x: [-1, 1], xSpread: 1, y: [0, 1] }}
	{template}
/>
