...
describe('Translation Helpers', () => {
	...
	describe('getInitialLanguageMap', () => {
		...
		it('설정 파일을 읽고 언어 맵을 올바르게 초기화해야 한다', () => {
			...
			expect(vi.mocked(fs.readFileSync)).toHaveBeenCalledWith('mock/path/to/settings.json', 'utf8')
	...
	describe('prepareTranslationPayload', () => {
		...
		beforeEach(() => {
			...
			vi.mocked(generateKeyNumberFunctions)
				.mockClear()
				.mockImplementation(() => ({
	...
	describe('translateOneLanguageMessages', () => {
		...
		it('성공적인 번역 과정을 올바르게 조율하고 결과와 새 사전을 반환해야 한다', async () => {
			...
			const mockApiTranslatedMessages = {
				// getTranslatedMessages 가 반환할 번호 매겨진 메시지
				1: 'translated val1',
				2: 'translated val2',
			}
			const mockApiNewDictionary = {
				// getTranslatedMessages 가 반환할 새 사전 항목
				wordC: 'defC',
			}
			const expectedRestoredMessages = {
				// integrateTranslatedMessages 내부에서 계산됨
