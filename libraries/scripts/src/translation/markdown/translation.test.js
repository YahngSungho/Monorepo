...
import { convertMarkdownFiles, getFiles, getTranslatedLanguageMap, saveFiles } from './translation.js';
...
describe('getTranslatedLanguageMap 함수', () => {
	...
	const mockTranslateResultDe = { // de는 targetLanguageMap에 포함되지 않을 수 있음 (ko, en 기반)
	...
	beforeEach(() => {
		...
		vi.mocked(translateOneLanguageMessages).mockImplementation(async (language) => {
            ...
            } else if (language === 'de') { // else if 사용
	...
	it('각 대상 언어에 대해 translateOneLanguageMessages를 호출하고 결과를 집계해야 한다', async () => {
		// 준비 (Arrange)
		const messageMap = { // ko, en 제공, fr, de, ja는 번역 대상 가정
			ko: { key1: '안녕', key2: '세상' },
...
describe('saveFiles 함수', () => {
    ...
    it('번역 파일 저장 기능 테스트', async () => {
        ...
        const explanations = { 'comp/text': 'Greeting', 'comp/other': 'Object', 'comp/extra': 'Additional' };
        const languageMessageMap_ko = { 'comp/text': '안녕', 'comp/other': '세상', 'comp/extra': '추가' };
        const languageMessageMap_en = { 'comp/text': 'Hello', 'comp/other': 'World', 'comp/extra': 'Extra' };
        ...
        expect(vi.mocked(getNewCache)).toHaveBeenCalledWith({ ko: languageMessageMap_ko, en: languageMessageMap_en }, explanations);
    ...
    it('빈 번역에 대한 캐시 저장 기능 테스트', async () => {
        ...
        if (JSON_stringify_spy.mock.calls.length > 0) {
            console.log("JSON.stringify 첫 번째 호출 인자:", JSON.stringify(JSON_stringify_spy.mock.calls[0]));
        }
