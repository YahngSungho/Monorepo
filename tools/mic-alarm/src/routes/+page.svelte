<script>
	...
	async function startMic_action() {
		try {
			...
			permissionError = (err && err.message) ? err.message : "마이크 권한 거부 또는 초기화 실패";
			isReady = false;
			isMonitoring = false;
		}
	}
		...
		<div class="row">
			...
			<div class="value">
				{#if isPaused}
					<strong> - </strong> 초
				{:else}
					<strong>{thresholdSeconds - secondsSinceHeard}</strong> 초
				{/if}
			</div>
		</div>
		<div class="row">
			...
			<div class="value">
				{#if isPaused}
					<strong> - </strong> 초
				{:else}
					<strong>{secondsSinceHeard}</strong> 초
				{/if}
			</div>
		</div>
	</section>
...
<style>
	...
	.btn {
		...
		&:hover { background: var(--accent, rgb(238 238 238)); }
